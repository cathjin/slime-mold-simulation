...
def optimize_routes(colour_grid, mold_grid):
    new_colour_grid = np.copy(colour_grid)
    new_mold_grid = np.copy(mold_grid)
    for a in range(0, len(known_oats)):
        ...
        for i in range(0, mold_grid.shape[0]):
            for j in range(0, mold_grid.shape[1]):
                if non_diagonal_num_live_neighbours(i,j) < 2:
                    new_mold_grid[i,j] = 0
                    if items_grid[i,j] == 0:
                ...
                or distance(oatx,oaty,i,j) + distance(32,32, i,j) > distance (oatx,oaty,32,32) + 3):
                    new_mold_grid[i,j] = 0
                    new_colour_grid[i,j,0] = 0
                    new_colour_grid[i,j,1] = 0
                    new_colour_grid[i,j,2] = 0
...
for i in range(0, 32):
    ...
    colour_grid, mold_grid = optimize_routes(colour_grid, mold_grid)
    time.sleep(1)