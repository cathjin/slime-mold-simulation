        ...
        for j in range(0, mold_grid.shape[1]):
            ...
            if mold_grid[i,j] == 0:
                if (live_neighbours >= 1) and items_grid[i][j] != -1:
                    new_mold_grid[i,j] = 1
                    if items_grid[i,j] == 0:
                        new_colour_grid[i,j, 0] = 237 * new_mold_grid[i,j] / 7
                        new_colour_grid[i,j, 1] = 224 * new_mold_grid[i,j] / 7
                        new_colour_grid[i,j, 2] = 36  * new_mold_grid[i,j] / 7
            if items_grid[i][j] == 1:
                if new_mold_grid[i,j] != 0 and not((i,j) in known_oats):
                    new_mold_grid[i,j] = mold_grid[i,j] + 15
                    path_found [i,j] = 1
                    known_oats.insert(0,(i,j))
                    for k in range(1, len(known_oats)):
                        find_path_wrapper(known_oats[0],[known_oats[k]])
            ...
            if oat_found == True and items_grid[i,j] != -1:
                ...
                if items_grid[i,j] == 0:
                    new_colour_grid[i,j, 0] = min(255, 237 * new_mold_grid[i,j] / 7)
                    new_colour_grid[i,j, 1] = min(255, 224 * new_mold_grid[i,j] / 7)
                    new_colour_grid[i,j, 2] = min(255, 36  * new_mold_grid[i,j] / 7)
...
def neighbours_min_distance(grid, x,y, destination):
    ...
    closest_index = -1
    for i in range(0, len(neighbours)):
        neighbourx, neighboury = neighbours[i]
        if distance(destination, neighbours[i]) < min_distance and items_grid[neighbourx, neighboury] != -1:
...
def find_path(grid, destination, path):
    ...
    closest_neighbour_index = neighbours_min_distance(grid, currentx,currenty, destination)
    print (closest_neighbour_index)
    if len(neighbours) > 0 and closest_neighbour_index != -1:
